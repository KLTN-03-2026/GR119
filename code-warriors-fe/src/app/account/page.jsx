...
import AvatarUploadForm from './avatar/page';
const ProfilePage = () => {
    ...
    const [showAvatarUploadForm, setShowAvatarUploadForm] = useState(false);
    useEffect(() => {
        fetchUserInfo();
    }, []);
    ...
    const handleSave = async (field) => {
        try {
            ...
            const response = await axios.patch('http://localhost:8080/api/v1/user/updateProfile', updateData, {
                withCredentials: true,
                headers: {
    ...
    const renderFieldInput = (field, value) => {
        if (field === 'gender') {
            ...
            // Trong phần renderFieldInput, thay đổi xử lý cho trường birthday
            return (
                <input
                    type="text"
                    value={tempValue}
            ...
            <div className="flex items-center gap-2 ml-5 w-9/10">
                {editingField === field ? (
                    <div className="flex items-center gap-2 w-full">
                        ...
                        <button onClick={() => handleSave(field)} className="text-green-500 hover:text-green-700 p-2">
                            <Save size={18} />
                        </button>
                        <button onClick={handleCancel} className="text-red-500 hover:text-red-700 p-2">
                            <X size={18} />
                        </button>
                    </div>
                ) : (
                    ...
                    <div className="relative">
                        ...
                        {showAvatarUploadForm && (
                            <AvatarUploadForm
                                ...
                                onCancel={() => setShowAvatarUploadForm(false)}
                            />
                        )}
                        <label
                            onClick={() => setShowAvatarUploadForm(true)}
